...
public class RobotContainer {
  ...
  //In port 0
  public PS4Controller controller = new PS4Controller(OperatorConstants.kDriverControllerPort);
  ...
  private void configureBindings() {
    ...
    controller.getCrossButton().whileTrue(driveTrainSS.fasterTurning());
    // Run the fasterTurning command only while X is held
    controller.getR2Button().whileTrue(shooterSS.shootNormal());
    // Run the shootNormal command only while right trigger is held
    controller.getCircleButton().whileTrue(loaderSS.load());
    // Run the load command only while B is held
    controller.getTriangleButton().toggleOnTrue(loaderSS.unload());
  ...
  // When A is pressed, shift bot aiming over to apriltag
    controller.cross().whileTrue(aimCommand);
