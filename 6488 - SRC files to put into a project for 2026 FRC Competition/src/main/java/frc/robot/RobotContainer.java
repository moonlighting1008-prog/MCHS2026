...
public class RobotContainer {
  ...
  private void configureBindings() {
    ...
    // Sets default command for driving
    controller.L1().whileTrue(driveTrainSS.fasterTurning());
    // Runs the fasterTurning command only while Square is held
    controller.R2().whileTrue(shooterSS.shootNormal());
    // Runs the shootNormal command only while R2 is held
    controller.L2().whileTrue(shooterSS.reverseShooter());
    controller.circle().whileTrue(loaderSS.load());
    // Runs the load command only while Circle is held
    controller.triangle().toggleOnTrue(loaderSS.unload());
    // Runs the unload command when Triangle is pressed, stop when pressed again
